...
backend_args = None
train_pipeline = [
    dict(type='LoadRSImageFromFile', bands_index=[7, 6, 5, 4, 3, 2, 1, 0]),
...
# max_epochs = 12
train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=250, val_interval=5)
...
img_bands_index = dict(bands_index=[7, 6, 5, 4, 3, 2, 1, 0])
img_norm_method = dict(method='min-max')
metainfo = dict(classes=('tree', ), palette=[(220, 20, 60)])
seed = 42
gpu_ids = range(0, 2)
device = 'cuda'
work_dir = './log/dino_swim_b7+1'
launcher = 'pytorch'
